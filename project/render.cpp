...
CircularBuffer<float> gInputBuffer(kBufferSize);
int gHopCounter = 0;
std::vector<float> wavetable (512);

// Name of the sound file (in project folder)
std::string gFilename = "guitarstrings.wav"; 
...
void process_fft()
{
	...
	// Fill buffer with all past elements
	for (int i = 0; i < gFftSize; i++) {
		unwrappedBuffer.at(i) = gInputBuffer.get_element_N_ago(gFftSize - i - 1);
	}
